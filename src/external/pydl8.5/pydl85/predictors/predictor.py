...
def get_dot_body(
    treedict, parent=None, left=True, feature_names=None, class_names=None
):
    gstring = ""
    id = str(uuid.uuid4())
    id = id.replace("-", "_")
    ...
    if "feat" in treedict.keys():
        ...
        if parent is None:
            ...
            gstring += get_dot_body(
                treedict["left"], id, True, feature_names, class_names
            )
            gstring += get_dot_body(
                treedict["right"], id, False, feature_names, class_names
            )
        else:
            gstring += "node_" + id + ' [label="{{feat|' + str(feat) + '}}"];\n'
            gstring += (
        ...
        val = (
            str(int(treedict["value"]))
            if treedict["value"] - int(treedict["value"]) == 0
            else str(round(treedict["value"], 3))
        )
        if class_names is not None:
            val = class_names[int(val)]
            val = html.escape(val)
        err = (
            str(int(treedict["error"]))
            if treedict["error"] - int(treedict["error"]) == 0
            else str(round(treedict["error"], 2))
        )
        # maxi = max(len(val), len(err))
        # val = val if len(val) == maxi else val + (" " * (maxi - len(val)))
        # err = err if len(err) == maxi else err + (" " * (maxi - len(err)))
        gstring += (
            "leaf_" + id + ' [label="{{class|' + val + "}|{error|" + err + '}}"];\n'
        )
        gstring += (
            "node_" + parent + " -> leaf_" + id + " [label=" + str(int(left)) + "];\n"
        )
    return gstring
...
class DL85Predictor(BaseEstimator):
    ...
    def _more_tags(self):
        return {"X_types": "categorical", "allow_nan": False}
    ...
    def fit(self, X, y=None):
        ...
        user_fast_func = (
            self.fast_error_function
        )  # input: support per class, output: label, error
        ...
        solution_str = dl85Optimizer.solve(
            data=X,
            target=y,
            ...
            predictor=predict,
            cachetype=(
                dl85Optimizer.CacheType.CacheTrieItemset
                if self.cache_type == Cache_Type.Cache_TrieItemset
                else (
                    dl85Optimizer.CacheType.CacheHashItemset
                    if self.cache_type == Cache_Type.Cache_HashItemset
                    else dl85Optimizer.CacheType.CacheHashCover
                )
            ),
            cachesize=self.maxcachesize,
            wipetype=(
                dl85Optimizer.WipeType.All
                if self.wipe_type == Wipe_Type.All
                else (
                    dl85Optimizer.WipeType.Subnodes
                    if self.wipe_type == Wipe_Type.Subnodes
                    else dl85Optimizer.WipeType.Reuses
                )
            ),
        ...
        self.tree_ = (
            json.loads(solution[-8].split("Tree: ")[1])
            if "No such tree" not in solution[-8]
            else None
        )
        ...
        if self.tree_ is None:  # No solution
            if not self.timeout_:
                print(
                    "DL8.5 fitting: Solution not found. However, a solution exists with error greater than or equal to the max error (exlusive upper bound) you specify. Please increase your bound if you want to find it."
                )
            else:
                print(
                    "DL8.5 fitting: Timeout reached without solution. Please increase the time limit and/or the max error (exlusive upper bound)"
                )
        else:
            if not self.quiet:
                if not self.timeout_:
                    ...
                    print(
                        "DL8.5 fitting: Timeout reached. The solution found may not be optimal"
                    )
        ...
        self.base_tree_ = (
            self.get_tree_without_transactions_and_probas()
            ...
            if transactions is None:
                ...
                if y is not None:
                    unique, counts = np.unique(
                        y[node["transactions"]], return_counts=True
                    )
                ...
                if positive:
                    ...
                    current_transactions = set(transactions).intersection(
                        positive_vector
                    )
                    ...
                    if "feat" in node.keys():
                        recurse(current_transactions, node["left"], node["feat"], True)
                        recurse(
                            current_transactions, node["right"], node["feat"], False
                        )
                else:
    ...
    def get_tree_without_transactions(self):
        ...
        def recurse(node):
            if "transactions" in node and (
                "feat" in node.keys() or "value" in node.keys()
            ):
    ...
    def get_tree_without_transactions_and_probas(self):
        ...
        tree = dict(copy.deepcopy(self.tree_))
        recurse(tree)
        return tree
